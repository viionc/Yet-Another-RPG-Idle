...
function App() {
  const dialogues = useSelector((state: GameState) => state.dialogues)
  const { navTab } = useSelector((state: GameState) => state.playerSettings)
  const { currentTutorialId } = useSelector((state: GameState) => state.playerUnlockedContent)

  return (
    <>
      {dialogues.currentNpcId !== null ? <DialogueModal id={dialogues.currentNpcId} /> : null}
      {dialogues.questCompletedIdForModal !== null ? (
        <QuestRewardModal id={dialogues.questCompletedIdForModal} />
      ) : null}
      {currentTutorialId !== null ? <TutorialModal tutorialId={currentTutorialId} /> : null}
      <GameLoopComponent />
      <Header />
      <main
        className="container grid grid-cols-4 gap-2 text-white"
        style={{ gridTemplateRows: '20rem 5rem 20rem' }}
      >
        <StatsPanel />
        {navTab === 'Main' ? (
          <>
