...
import { BattleStateEnemyProps } from '../../store/battle-state'
import { elementTypeToString } from '../../consts/skills'
import Spinner from '../ui/Spinner'

type EnemyComponentProps = {
  weaknessesUnlocked: boolean
  ...
  return (
    <div>
      <div className="absolute z-10 top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-8 bg-black border-2 border-yellow-500 flex justify-center items-center">
        ...
        <span
          className="z-20 h-full absolute top-0 left-0 bg-red-500"
          style={{ width: (enemy.currentHp / enemy.maxHp) * 100 + '%' }}
        ></span>
      </div>
      {weaknessesUnlocked && (
        <img
          src={`./icons/${elementTypeToString[weakness].toLowerCase()}Weakness.png`}
