...
function StatsPanel() {
  ...
  return (
    <section className="grid-span-1 p-2 bg-neutral-800 h-[20rem] shadow-md rounded-md">
      <h1 className="mb-2">Stats:</h1>
      <ul>
        <li className="flex gap-2 items-center">
          Level: {playerStats.level}
          {playerStats.unspentSkillPoints && <span> - Skill Points: {playerStats.unspentSkillPoints}</span>}
        </li>
        <li className="text-cyan-500">
          Xp: {playerStats.experience.toLocaleString('en-US')}/
          {calculateXp(playerStats.level + 1).toLocaleString('en-US')}
        </li>
        ...
        <li>
          ...
          <span className="text-blue-500">{playerStats.currentManaRegenTimer}</span>
        </li>
