...
function Building({ tab, setSelectedTab }: BuildingProps) {
  return (
    <div
      className="bg-no-repeat bg-center bg-cover w-full h-full rounded-md relative "
      style={{ backgroundImage: `url(${tab.url})` }}
    >
      <CloseButton
        position="top-right"
        callback={() => setSelectedTab(null)}
      />
      ...
      {tab.objectsIds &&
        tab.objectsIds.map((object) => {
          if (object.type === TownObjectIds.zoneEntrance && !checkIfMeetsRequirements(object.requirement)) return
