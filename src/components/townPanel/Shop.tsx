...
import { updateStock } from '../../store/shops/shops.slice'
...
function Shop({ tab, closeShop, id }: ShopBuildingProps) {
  ...
  const shops = useSelector((state: GameState) => state.shops)
  const { currentShopRefreshCooldown } = useSelector((state: GameState) => state.playerStats)
    ...
    >
      <div className="w-full h-full bg-black bg-opacity-60 rounded-md p-4">
        <CloseButton
          ...
          callback={closeShop}
        />
        <AmountMultiplierList
          amountMultiplier={amountMultiplier}
          setAmountMultiplier={setAmountMultiplier}
        />
