...
function Header() {
  ...
  const { unspentSkillPoints, level } = useSelector((state: GameState) => state.playerStats)
  const playerUnlockedContent = useSelector((state: GameState) => state.playerUnlockedContent)
          ...
          {level > 1 ? (
            ...
            >
              ...
              {unspentSkillPoints > 0 ? (
                <span className="text-yellow-500 group-hover:text-black">({unspentSkillPoints})</span>
              ) : null}
            </li>
          ) : null}
